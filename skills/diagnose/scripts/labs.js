...
const GENERATED_FILE_RE = /\.(gen|generated)\.[\w]+$|_pb2(_grpc)?\.pyi?$|\.pb\.go$/;
const GENERATED_MARKER_RES = [/@generated\b/, /\bcode generated by\b[\s\S]*?\bdo not edit\b/i, /\bautogenerated\b[\s\S]*?\bdo not edit\b/i];
...
function isGeneratedHeader(lines) {
  const header = lines.slice(0, GENERATED_HEADER_LINES).join('\n');
  return GENERATED_MARKER_RES.some((re) => re.test(header));
}
...
function scanFile(root, rel) {
  ...
  if (isGeneratedHeader(lines)) return null;
